...
public class ProductInfoPageUsingExcel extends BaseTest{
	...
	@DataProvider
	public Object[][] getProductTestDataFromXl() {
		
	Object[][]	prodData =  ExcelUtils.getTestData(AppConstants.REGISTER_SHEET_NAME);
	return prodData;
			};
	
	@Test(dataProvider = "getProductTestDataFromXl")
	public void productHeaderTest(String SearchKey , String MainProductSelected) {
		
	resultPage =	accPage.performSearch(SearchKey);
	prodInfoPage = resultPage.selectProd(MainProductSelected);
	String actualHeader = 	prodInfoPage.getProductHeader();
	Assert.assertEquals(actualHeader, MainProductSelected);
...
//execute all the assertion  even if any of the assertions fails  		
		...
		softAssert.assertEquals(actProdInfo.get("Reward Points"), "80079");
